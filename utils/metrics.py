...
def wasserstein_loss(box1, box2, xywh=True, eps=1e-7, constant=9.0):
    if xywh:  # transform from xywh to xyxy
        b1x1, b1y1, b1w1, b1h1 = box1.chunk(4, -1)
        b2x1, b2y1, b2w1, b2h1 = box2.chunk(4, -1)

    else:  # x1, y1, x2, y2 = box1
        b1x1s, b1y1s, b1x2s, b1y2s = box1.chunk(4, -1)
        b2x1s, b2y1s, b2x2s, b2y2s = box2.chunk(4, -1)
        b1x1 = (b1x1s + b1x2s)/2
        b1y1 = (b1y1s + b1y2s) / 2
        b1w1 = b1x2s - b1x1s+ eps
        b1h1 = b1y2s - b1y1s + eps
        b2x1 = (b2x1s + b2x2s)/2
        b2y1 = (b2y1s + b2y2s) / 2
        b2w1 = b2x2s - b2x1s + eps
        b2h1 = b2y2s - b2y1s + eps

    wpo = (2 * torch.exp(-b2w1))/(torch.exp(-b2w1)+torch.exp(-b2h1))
    hpo = (2 * torch.exp(-b2h1)) / (torch.exp(-b2w1) + torch.exp(-b2h1))

    center_distance = wpo*((b1x1-b2x1)**2)+hpo*((b1y1-b2y1)**2) + eps
    #center_distance = ((b1x1 - b2x1) ** 2) +((b1y1 - b2y1) ** 2) + eps

    wh_distance = (wpo*(b1w1 - b2w1) ** 2 + hpo*(b1h1 - b2h1) ** 2) / 4
    #wh_distance = ((b1w1 - b2w1) ** 2 + (b1h1 - b2h1) ** 2) / 4
